...
Cli::Cli(std::string flights_path, std::string airports_path) : routes_(db_) {
    db_.load(flights_path);
    airports_.load(airports_path);
    std::cout << "已加载航班 " << db_.size() << " 条，机场 " << airports_.airports().size()
              << " 个。" << std::endl;
}

void Cli::print_flight(const Flight& f) {
    std::cout << "  ID=" << f.id
              << " 航班号=" << f.flight_no
...
void Cli::print_routes(const std::vector<std::vector<int>>& routes) {
    if (routes.empty()) {
...
void Cli::show_stats() const {
    const FlightStats s = db_.stats();
    print_separator();
    std::cout << "【实验一】数据统计" << std::endl;
    if (s.earliest_departure) {
        std::cout << "起飞最早: ";
        print_flight(*s.earliest_departure);
        std::cout << "起飞最晚: ";
        print_flight(*s.latest_departure);
        std::cout << "飞行最短: ";
        print_flight(*s.shortest_duration);
        std::cout << "飞行最长: ";
        print_flight(*s.longest_duration);
        std::cout << "票价最低: ";
        print_flight(*s.cheapest);
        std::cout << "票价最高: ";
        print_flight(*s.most_expensive);
    } else {
        std::cout << "  无活跃航班数据。" << std::endl;
    }
}

void Cli::query_direct() const {
    print_separator();
    std::cout << "【实验一】直达航班查询与标记" << std::endl;
    int from = read_int("请输入出发机场 ID: ");
    int to = read_int("请输入到达机场 ID: ");
    const auto direct = db_.direct_flights(from, to);
    if (direct.empty()) {
    ...
    const BestFlights best = db_.best_flights(from, to);
...
void Cli::add_flight() {
    ...
    try {
        db_.add(FlightDatabase::parse_row(fields));
...
void Cli::modify_flight() {
    ...
    try {
        db_.update(FlightDatabase::parse_row(fields));
...
void Cli::suspend_resume() {
    ...
    if (choice == 1) {
        db_.suspend_airport(id);
        std::cout << "机场 " << id << " 已暂停。" << std::endl;
    } else if (choice == 2) {
        db_.resume_airport(id);
...
void Cli::max_flights() const {
    ...
    std::cout << "最多可乘坐 " << res.max_count << " 次航班，共 " << res.routes.size()
              << " 条最长路线：" << std::endl;
    print_routes(res.routes);
}

void Cli::search_airport() const {
    print_separator();
    std::cout << "【实验四】机场名称近似搜索" << std::endl;
    const std::string query = read_line("请输入关键词: ");
    const auto results = airports_.search_by_name(query, 5);
...
void Cli::recommend_province() const {
    print_separator();
    ...
    const auto rec = airports_.recommend_same_province(db_, from, to);
    if (rec.empty()) {
...
void Cli::busiest_airport() const {
    print_separator();
    ...
    const auto traffic = airports_.busiest_airports(db_, dep_win, arr_win);
    if (traffic.empty()) {
    ...
    for (const auto& t : traffic) {
        const Airport* a = airports_.find_by_id(t.airport_id);
...
void Cli::connectivity() const {
    ...
    const auto routes = routes_.connectivity(from, to, 1, dep_win, arr_win);
    std::cout << "可行乘机方案共 " << routes.size() << " 条：" << std::endl;
    print_routes(routes);
}

void Cli::optimal_route() const {
    print_separator();
    ...
    const auto routes = routes_.optimal_routes(from, to, criterion);
...
int Cli::run() {
    while (true) {
        ...
        switch (choice) {
            ...
            case 2: query_direct(); break;
            case 3: add_flight(); break;
            case 4: remove_flight(); break;
            case 5: modify_flight(); break;
            case 6: batch_apply(); break;
            case 7: suspend_resume(); break;
            case 8: max_flights(); break;
            case 9: search_airport(); break;
            case 10: recommend_province(); break;
            case 11: busiest_airport(); break;
            case 12: connectivity(); break;
            case 13: optimal_route(); break;
